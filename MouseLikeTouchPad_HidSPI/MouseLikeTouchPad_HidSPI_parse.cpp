#include "MouseLikeTouchPad_HidSPI.h"
...
void MouseLikeTouchPad_HidSPI_parse_init(DEV_EXT* pDevContext)
{
...
void MouseLikeTouchPad_HidSPI_parse(DEV_EXT* pDevContext, UINT8* data, LONG length)
{
	...
	if (pDevContext->bPtpReportCollection) {//触摸板集合，手势模式判断
		...
		if (!Mouse_Wheel_mode) {//以指针手指释放为滚轮模式结束标志，下一帧bPtpReportCollection会设置FALSE所以只会发送一次构造的手势结束报告
			...
			RegDebug(L"MouseLikeTouchPad_HidSPI_parse bPtpReportCollection bGestureCompleted0", NULL, status);
			...
			RegDebug(L"MouseLikeTouchPad_HidSPI_parse bPtpReportCollection bGestureCompleted1", NULL, status);
		...
		if (!pDevContext->bGestureCompleted) {//手势未结束，正常发送报告
			RegDebug(L"MouseLikeTouchPad_HidSPI_parse bPtpReportCollection bGestureCompleted2", NULL, status);
			//发送ptp报告
			pEvt.collectionType = PTP_CollectionType;//
